...
def main(args):
    ...
    n_links = len([f for f in h5file.get_fragment_dataset_paths(1) if f.split("_")[-1] == "WIBEth"]) # exclude other fragments in the record that are not WIB frames
    ...
    with open(out_name, "wb") as bf:
        ...
        for i in range(args.n_records):
            ...
            fragments = [f for f in fragments if f.split("_")[-1] == "WIBEth"] # exclude other fragments in the record that are not WIB frames
            ...
            for j in range(n_frames):
                ...
                data = fddetdataformats.WIBEthFrame(f.get_data(j * WIBEthFrame_size)) # unpack fragment to WIBEthFrame
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description = 'Python script to write WIBEth frames from DUNE-DAQ HDF5 files to binary files.')
