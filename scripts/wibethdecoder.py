...
def main(filenames, nrecords, nskip, channel_map, print_headers, print_adc_stats, check_timestamps, det, quiet):
    ...
    for filename in filenames:
        ...
        print(f'Will process {len(records_to_process)} of {len(records)} records in {filename}.')
        ...
        for r in records_to_process:
            ...
            for gid in wib_geo_ids:
                ...
                if not quiet:
                    print(f'\tFound {n_frames} WIBEth Frames.')
            ...
            if check_timestamps:
                timestamps_frame0 = np.array([ fddetdataformats.WIBEthFrame(h5_file.get_frag(r,gid).get_data()).get_timestamp() for gid in wib_geo_ids ])
