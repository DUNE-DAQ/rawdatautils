 ...
 */
uint32_t
n_wib2_frames(daqdataformats::Fragment const& frag)
{
...
np_array_adc_data(void* data, int nframes)
{
  ...
  for (size_t i = 0; i < (size_t)nframes; ++i) {
    auto fr = reinterpret_cast<detdataformats::wib2::WIB2Frame*>(static_cast<char*>(data) +
                                                                 i * sizeof(detdataformats::wib2::WIB2Frame));
